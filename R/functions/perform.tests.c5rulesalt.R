...
#
############################################################################
perform.cv.c5rulesalt <- function(n, k, ds.id = "mod", subset = c(), transformation = c(), imputation.method = c(), training.only.complete = FALSE, testing.only.complete = FALSE, test.cases.remove = c(), cutoffs = 0.5, param.df, verbose = TRUE)
{
  ...
  for (test in 1:nrow(param.df)) 
  {
    ...
    for(foldInds.index in 1:n)
    {
      ...
      for (fold in 1:k)
      {
        ...
        set.seed(42);
        model <- C5.0(as.simple.formula(subset, "a_dm"), data = training, trials = 1, costs = param.costs, rules = TRUE, 
...
output.cv.results.c5rulesalt <- function(result, cutoffs, param.df, number.folds, root.path = "output/results/c5rulesALT", tst.id = "cv_mean_test", verbose = TRUE)
{
  ...
  result.dataframe <- data.frame(param, costs, minCases, fuzzyThreshold, earlyStopping, winnow, noGlobalPruning, balanced_accuracy.roc.mean, balanced_accuracy.roc.sd, sensitivity.roc.mean, sensitivity.roc.sd, specificity.roc.mean, specificity.roc.sd);
  result.dataframe <- result.dataframe[ order(-balanced_accuracy.roc.mean), ];
  fname <- sprintf("%s/result_c5rulesalt_%s.csv",output.folder.csv, tst.id);
...
perform.tests.c5rulesalt <- function(n, k, ds.id, subset = c(), transformation = c(), imputation.method = c(), training.only.complete = FALSE, testing.only.complete = FALSE, test.cases.remove = c(), tst.id, verbose = TRUE)
{
  ...
  param.df[1,] <- c(11, 250, FALSE, TRUE, TRUE, FALSE);
  param.df[2,] <- c(11, 260, FALSE, TRUE, FALSE, FALSE);
  param.df[3,] <- c(8, 64, FALSE, TRUE, TRUE, FALSE);
...
generalization.predict.c5rulesalt <- function (model, dataset.testing, tst.id, model.id = "orig", subset.id = "unico") {
  ...
  dir.create(output.folder, showWarnings = FALSE);
  result.dataframe <- data.frame("C5-RULES (class)", model.id, subset.id, auc, balanced_accuracy, sensitivity, specificity);
