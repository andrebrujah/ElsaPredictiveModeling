...
generalization.predict.rf <- function (model, dataset.testing, tst.id, model.id = "orig", subset.id = "unico") {
  best.cutoff <- 0.13;
  pred.aux <-  predict(model, newdata = dataset.testing, type='prob');
  pred.resp <- pred.aux[,"0"]; 
  ...
  dir.create(output.folder, showWarnings = FALSE);
  result.dataframe <- data.frame("Random Forest", model.id, subset.id, auc, balanced_accuracy, sensitivity, specificity);
  ...
  output.folder.roc_curves <- file.path(output.folder, "roc_curves");
  dir.create(output.folder.roc_curves, showWarnings = FALSE);
  fname <- sprintf("%s/generalization_%s_rf.png",output.folder.roc_curves, tst.id);
  png(filename=fname);
