...
#
############################################################################
perform.cv.nbalt <- function(n,k,  ds.id = "mod", subset = c(), transformation = c(), imputation.method = c(), training.only.complete = FALSE, testing.only.complete = FALSE, test.cases.remove = c(), cutoffs = 0.5, param.df = data.frame(laplace = 3), verbose = TRUE)
{ 
  test <- 0;
  result <- list();
...
output.cv.results.nbalt <- function(result, cutoffs, param.df, number.folds, root.path = "output/results/nbalt", tst.id = "cv_mean_test", verbose = TRUE)
{
  ...
  result.dataframe <- data.frame(param, laplace, balanced_accuracy.roc.mean, balanced_accuracy.roc.sd,
  ...
  fname <- sprintf("%s/result_nbalt_%s.csv",output.folder.csv, tst.id);
...
generalization.predict.nbalt <- function(model, dataset.testing, tst.id, model.id = "orig", subset.id = "unico") {
  ...
  dir.create(output.folder, showWarnings = FALSE);
  result.dataframe <- data.frame("Naive Bayes (class)", model.id, subset.id, auc, balanced_accuracy, sensitivity, specificity);
