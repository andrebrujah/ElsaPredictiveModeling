...
perform.cv.glm <- function(n,k, ds.id = "mod", subset = c(), transformation = c(), imputation.method = c(), training.only.complete = FALSE, testing.only.complete = FALSE, test.cases.remove = c(), cutoffs = 0.5, param.df = data.frame(epsilon = 1e-8, maxit = 25), verbose = TRUE)
{
  ...
  for (test in 1:nrow(param.df)) 
  {
    ...
    param.epsilon <- param.df$epsilon[test];
    param.maxit <- param.df$maxit[test];
    ...
    for(foldInds.index in 1:n)
    {
      ...
      for (fold in 1:k)
      {
        ...
        set.seed(42);
        model <- glm(as.simple.formula(subset, "a_dm"), family=binomial(link="logit"), data=training, control=list(epsilon = param.epsilon, maxit = param.maxit));  
        set.seed(42);
        pred.resp <- predict(model, testing[,-index.class], type='response');
...
output.cv.results.glm <- function(result, cutoffs, param.df, number.folds, root.path = "output/results/glm", tst.id = "cv_mean_test", verbose = TRUE)
{
...
generalization.predict.glm <- function (model, dataset.testing, tst.id, model.id = "orig", subset.id = "unico") {
  best.cutoff <- 0.11;
  pred.resp <- predict(model, dataset.testing, type='response');  
  notPredictedCases <- length(which(is.na(pred.resp)));
  if (notPredictedCases > 0) 
  {
    capture.output({cat("\n\nGLM:", 
  ...
  dir.create(output.folder, showWarnings = FALSE);
  result.dataframe <- data.frame("Regressão Logistica", model.id, subset.id, auc, balanced_accuracy, sensitivity, specificity);
